...
/// tests: it only runs once an async `@MainActor` test yields the main actor, and the host's
/// first-frame presentation SIGTRAPs the process mid-test on CI simulators (Xcode 16.x test
/// stack) — a launch-timing race, not any test's own logic (the trap lands on whichever async
/// test happens to be executing, so it moves run to run). So under tests we launch a bare
/// `UIApplication` with an inert delegate: no scene, no window, no launch work, nothing to
/// race. (On iOS 26 simulators a scene-less launch trips the UIScene-lifecycle assert, which is
/// why CI pins an iOS 18.5 simulator — the supported pairing for the default Xcode.)
///
