...
## 3. Foundation shipped (M1–M5b done; M5c-wiring in progress)
...
- **M5b (both repos, done):** offline glass-pane capture + elevation serverId resolver.
  ...
  - iOS-side verification: scratch-SPM/RED-mutation green per lane (no full-suite `xcodebuild test` report exists yet for M5b/M5c — see Verification status below).
- **M5c-wiring (iOS, in progress):** assign-existing-pane onto an elevation, and widening the M5b serverId resolver to surfaces.
  ...
  - In flight: assign-pane UI (wiring `ElevationActions.assignSurface`/`SurfaceActions` into a view) and surface-photo capture UI. Not yet composition-root-wired or committed as of this status update.
...
**Verification status.** Per-lane scratch-SPM (TDD, RED-mutation-checked) + `swiftc -typecheck` against the real generated OpenAPI client remains the evidence standard for the parallel M4/M5a/M5b/M5c build waves (house rule: no `xcodebuild` runs while concurrent agents share the tree, to avoid clashing). M5-integration wave (`aa9bd2b`/`3d71a98`/`ba9842b`) reviewed **CLEAN** (`m5-integration-review.md`) — composition-root wiring, sign-out purge across all 8 `@Model` types, and the pane-fraction/Face-ID fixes all independently verified, not merely re-stated. Backend: **5223 web tests passed** for the full `npm run test` run covering M5b (`m5b-backend-report.md`). No `.superpowers/sdd` report yet documents a full-suite `xcodebuild test`/simulator run for the iOS app through M5b/M5c — the earlier "131 unit tests green" full-suite claim in this document was flagged **unsubstantiated** by `BranchAuditScout`'s branch-readiness audit (`pr-draft.md`) and is retracted here rather than repeated or replaced with an unverified new number; a real `xcodebuild test` pass remains outstanding before merge.

**External blockers (unchanged, tracked since M3):** Apple Developer account for TestFlight signing — blocks any real-device or TestFlight distribution. Staging API base URL is still a placeholder in the Release configuration (F6 partially done: app-icon placeholder + release plumbing exist; the actual staging host is unknown pending web-side infra). Neither blocks continued development or testing on `feat/m4-offline`, only distribution.
...
| On-site capture (`/projects/[id]/capture`, survey mod) | HIGH (161 takeoff + survey) | WATCH — *deferred by brief (M5, own planning doc)* | Building/elevation read sync + field-added-elevation writes done (M5a); offline glass-pane capture + elevation serverId resolver done (M5b); assign-existing-pane + surface serverId sync-widening landed, assign/photo UI in flight (M5c-wiring). Room grouping deferred — backend gap | M5a ✅ / M5b ✅ / M5c 🔶 in progress |
...
- **M5 — Field records. In progress.** M5a: building/elevation read sync + surface links, plus field-added-elevation + pane-assignment writes (done). M5b: offline glass-pane capture + elevation serverId chain resolver, reviewed clean (done). M5c-wiring: assign-existing-pane resolver + surface serverId sync-widening (done); assign-pane and surface-photo UI (in progress). Glass readings, inspections, caulking flows, room grouping (backend gap), installer-view/work-order re-audit remain. *DoD: per-flow.*
