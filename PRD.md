...
**Verification status.** Per-lane scratch-SPM (TDD, RED-mutation-checked) + `swiftc -typecheck` against the real generated OpenAPI client was the evidence standard during the parallel M4/M5 build waves. The integrated app is now confirmed by a full-suite simulator run: **`xcodebuild test` — 174/174 unit tests + 6/6 UITest smokes green** (Login, WorkLog check-in/out, offline check-in, offline elevation-create, offline pane-capture, sign-out E2E), stable under `-test-iterations 2 -retry-tests-on-failure`. The Release **device** archive compiles (unsigned) and the Simulator build is signing-free. M5-integration reviewed CLEAN; backend 5223 web tests for M5b.

**External blockers.** Apple Developer account — **now available**; signing is wired (`project.yml`: `CODE_SIGN_STYLE=Automatic`, `DEVELOPMENT_TEAM=S67XP8A7UZ`) and a `workflow_dispatch` TestFlight release workflow is in place (`.github/workflows/release.yml` + `ci/exportOptions.plist`, cloud-signed via an App Store Connect API key). Remaining before a TestFlight build can be cut — all account-side, **not code**: (1) create the App Store Connect app record for `com.getdatasurge.sightline.field`; (2) generate an ASC API key and add repo secrets `ASC_KEY_ID` / `ASC_ISSUER_ID` / `ASC_KEY_P8_BASE64`; (3) **team reconcile** — the dev Mac's local cert is under team `Z7F8QUNXAY` (a personal team) while `S67XP8A7UZ` has no registered devices/profiles yet (local signed archive fails with "no profiles"; App Store cloud signing in CI does not need a device). Staging API base URL is still a Release placeholder (unknown host, pending web infra) and MUST be swapped before any TestFlight build reaches testers. None block continued development on `feat/m4-offline`.
